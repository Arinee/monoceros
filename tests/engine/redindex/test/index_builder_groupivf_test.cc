...
TEST_F(IndexBuilderIvfTest, TestNoFeature)
{
    char *buffer;
    //也可以将buffer作为输出参数
    ...
    SchemaParams schema = {
        {"DataType", "float"},
        {"Method","IP"}, {"Dimension", "256"}, {"IndexType", "GroupIvf"},
        {"schema.builder.memory.quota", "5000000"},
        {"mercury.general.index.contain_feature_profile", "false"},
        {"TrainDataPath", "tests/engine/redindex/test_data/coarse_group/"},
    ...
    mercury::core::GroupIvfIndex* core_index = dynamic_cast<mercury::core::GroupIvfIndex*>(loaded_index.core_index_.get());
    ...
    EXPECT_EQ(1170000, core_index->GetMaxDocNum());
}
