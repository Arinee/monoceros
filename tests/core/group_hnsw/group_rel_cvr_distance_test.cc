...
class GroupHnswRelCvrCustomTest: public testing::Test
{
public:
    void SetUp()
    {
        ...
        index_params_.set(PARAM_CUSTOMED_PART_DIMENSION, 32);
        index_params_.set(PARAM_CUSTOM_DISTANCE_METHOD, "RelCvr");
...
TEST_F(GroupHnswRelCvrCustomTest, TestInit) {
    char *buffer;
    //也可以将buffer作为输出参数
    ...
    // std::string input = "/data1/heiniao/data/hnsw/input";
    // std::ifstream infile(input.c_str());
    // std::string line;
    // int i = 0;
    // while(getline(infile,line)) {
    //     line.pop_back();
    //     builder->AddDoc(i++, 0, line);
    // }
    // infile.close();
    ...
    // IndexStorage::Handler::Pointer handler = index_storage->open("/data1/heiniao/data/hnsw/mercury_index.package", false);
    // ASSERT_TRUE(handler);
    // void *data = nullptr;
