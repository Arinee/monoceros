...
class GroupIvfPqTwoStageTest : public testing::Test
{
public:
    void SetUp()
    {
        ...
        index_params_.set(PARAM_TRAIN_DATA_PATH, "tests/core/group_ivf_pq/test_data_two_stage/");
...
TEST_F(GroupIvfPqTwoStageTest, TestBuild)
{
    ...
    GroupIvfPqIndex *core_index = dynamic_cast<GroupIvfPqIndex *>(loaded_index_p.get());
...
TEST_F(GroupIvfPqTwoStageTest, TestSearch)
{
    ...
    std::string data2_str =
        "-0.0393940 -0.0504650 -0.0875190 -0.0643030 -0.1378400 -0.0762630 0.0239965 0.0190530 -0.1256575 "
        "-0.2664550 -0.0005035 -0.2034175 0.1323655 0.0094145 0.0743665 0.0673560 0.0073090 -0.0673595 "
        "-0.0890205 -0.1717630 0.0272180 0.0023510 -0.0747015 -0.0523320 -0.1239750 -0.2387470 0.0524925 "
        "-0.0582860 0.0732185 -0.0070135 0.1633875 -0.0867170 0.0198160 -0.0796320 -0.0304365 0.1818590 "
        "-0.2894615 -0.0979785 0.0475245 0.1159425 0.0018870 0.1361200 -0.5312295 0.0517105 0.0648055 0.1056075 "
    ...
    EXPECT_NEAR(-0.313, context.Result().at(0).score, 0.001);
    ASSERT_EQ(1, context.Result().at(1).gloid);
    EXPECT_NEAR(-0.908, context.Result().at(1).score, 0.001);
    ASSERT_EQ(2, context.Result().at(2).gloid);
    EXPECT_NEAR(-0.908, context.Result().at(2).score, 0.001);
}
