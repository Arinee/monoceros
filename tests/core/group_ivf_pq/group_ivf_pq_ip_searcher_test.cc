...
class GroupIvfPqIPSearcherTest: public testing::Test
{
public:
    void SetUp()
        {
...
TEST_F(GroupIvfPqIPSearcherTest, TestSearch) {
    std::cout << "cwd is:" << getcwd(NULL, 0) << std::endl;
...
TEST_F(GroupIvfPqIPSearcherTest, TestIndexParams) {
    std::cout << "cwd is:" << getcwd(NULL, 0) << std::endl;
...
TEST_F(GroupIvfPqIPSearcherTest, TestDimensionNotMatch) {
    Index::Pointer index1 = factory_.CreateIndex();
    mercury::core::GroupIvfPqIndex* core_index1 = dynamic_cast<mercury::core::GroupIvfPqIndex*>(index1.get());
...
TEST_F(GroupIvfPqIPSearcherTest, TestParallel) {
    ...
    mercury::core::GeneralSearchContext context4(index_params);
    search_str = "200&0:0#50;1:11#50;2:200#50||" + DataToStr(data);
    QueryInfo query_info4(search_str);
    ASSERT_TRUE(query_info4.MakeAsSearcher());
    ret_code = index_searcher->Search(query_info4, &context4);
    ASSERT_EQ(ret_code, 0);
    ASSERT_EQ(100, context4.Result().size());
    LOG_INFO("parallel with mixed group success");
    
    clearenv();
}

TEST_F(GroupIvfPqIPSearcherTest, TestRecallMode) {
    Index::Pointer index1 = factory_.CreateIndex();
    mercury::core::GroupIvfPqIndex* core_index1 = dynamic_cast<mercury::core::GroupIvfPqIndex*>(index1.get());
