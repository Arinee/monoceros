...
class DiskVamanaIndexTest: public testing::Test
{
public:
    void SetUp()
    {
        // test using random mock data:
        mem_index_mock_prefix_ = "tests/core/vamana/test_data/disk_in_mem_index_mock_R32_L50_A1.2";
        disk_index_mock_prefix_ = "tests/core/vamana/test_data/disk_in_SSD_index_mock_R32_L50_A1.2";
        ...
        mem_index_sift_learn_prefix_ = "tests/core/vamana/test_data/disk_in_mem_index_sift_learn_R32_L50_A1.2";
        disk_index_sift_learn_prefix_ = "tests/core/vamana/test_data/disk_in_SSD_index_sift_learn_R32_L50_A1.2";

        size_t data_num, data_dim;
        get_bin_metadata(base_filename_, data_num, data_dim);

        sift_index_params_.set(PARAM_DATA_TYPE, "float");
...
TEST_F(DiskVamanaIndexTest, TestCreate) {
...
TEST_F(DiskVamanaIndexTest, TestAddBase) {
    ...
    index->BaseIndexAdd(0, 0, query_info.GetVector(), query_info.GetVectorLen());
    ...
    alloc_aligned(((void **)&vec), index_mata.sizeofElement(), 8 * data_size);

    index->GetBaseVec(0, vec);

    ASSERT_EQ(vec[3], -0.032322f);

    ASSERT_EQ(vec[126], -0.051279f);

    ASSERT_EQ(1, index->GetBaseVecNum());

    index->BaseIndexAdd(1, 0, query_info.GetVector(), query_info.GetVectorLen());

    ASSERT_EQ(2, index->GetBaseVecNum());

    index->BaseIndexAdd(100, 0, query_info.GetVector(), query_info.GetVectorLen());

    ASSERT_EQ(3, index->GetBaseVecNum());
}

TEST_F(DiskVamanaIndexTest, TestBuildVamanaException) {

    GTEST_SKIP() << "Skipping DiskVamanaIndexTest.TestBuildVamanaException";
    ...
    EXPECT_THROW({
        ...
        {
            EXPECT_STREQ("Duplicate Data Exception", e.what());
            throw;
        }
    }, std::runtime_error);
}

TEST_F(DiskVamanaIndexTest, TestBuildVamanaMem) {
    ...
    ASSERT_EQ(get_file_size(mem_index_mock_prefix_ + ".data"), 5120008);

}

TEST_F(DiskVamanaIndexTest, TestCreateDiskLayout) {
    ...
    ASSERT_EQ(get_file_size(disk_index_mock_prefix_), 6832128);

}

TEST_F(DiskVamanaIndexTest, TestSiftLearnPQAndCluster) {

    GTEST_SKIP() << "Skipping DiskVamanaIndexTest.TestSiftLearnPQAndCluster";

    sift_index_params_.set(PARAM_VAMANA_INDEX_BUILD_IS_PARTITION, false);
    ...
    size = index->GetVamanaCentroidResource().getRoughMeta().elemSize / dim;

    ASSERT_EQ(4, size);

    const void *centroid_vec = index->GetVamanaCentroidResource().getValueInRoughMatrix(0, 1);

    EXPECT_NEAR(21.3411f, *((float *)((char *)centroid_vec + 0 * size)), 0.0001);
    EXPECT_NEAR(16.1287f, *((float *)((char *)centroid_vec + ((int)dim - 1) * size)), 0.0001);

    centroid_vec = index->GetVamanaCentroidResource().getValueInRoughMatrix(0, 2);

    EXPECT_NEAR(23.8947f, *((float *)((char *)centroid_vec + 0 * size)), 0.0001);
    EXPECT_NEAR(11.7824f, *((float *)((char *)centroid_vec + ((int)dim - 1) * size)), 0.0001);
    ...
    ASSERT_EQ(6531648, dump_size);

    ASSERT_TRUE(dump_data != nullptr);

    index->Load(dump_data, dump_size);

    fragmentCenter = index->GetPqCentroidResource().getValueInIntegrateMatrix(31, 255);
    size = index->GetPqCentroidResource().getIntegrateMeta().elemSize / 4;
    ASSERT_EQ(4, size);
    EXPECT_NEAR(64.5647f, *((float *)((char *)fragmentCenter + 0 * size)), 0.0001);
    EXPECT_NEAR(80.2931f, *((float *)((char *)fragmentCenter + 1 * size)), 0.0001);
    EXPECT_NEAR(10.6983f, *((float *)((char *)fragmentCenter + 2 * size)), 0.0001);
    ...
    centroid_vec = index->GetVamanaCentroidResource().getValueInRoughMatrix(0, 1);
...
TEST_F(DiskVamanaIndexTest, TestSiftLearnDumpPqLocal) {
    ...
    std::string pq_file_name = std::string(get_current_dir_name()) + "/vamana_pq.index";

    index->DumpPqLocal(pq_file_name);

    ASSERT_TRUE(file_exists(pq_file_name));

    ASSERT_EQ(get_file_size(pq_file_name), 6531648);

}

TEST_F(DiskVamanaIndexTest, TestSiftLearnPartitionBuildAndMerge) {
    ...
    index->CreateDiskLayout(std::string(get_current_dir_name()) + "/vamana_partition_ori.data", 
                            std::string(get_current_dir_name()) + "/vamana_partition_merged_mem.index", 
                            std::string(get_current_dir_name()) + "/vamana_partition_disk.index");

    ASSERT_TRUE(file_exists(std::string(get_current_dir_name()) + "/vamana_partition_disk.index"));

    ASSERT_EQ(get_file_size(std::string(get_current_dir_name()) + "/vamana_partition_disk.index"), 68272128);
...
TEST_F(DiskVamanaIndexTest, TestSiftLearnLoadAndSearchPartition) {
    ...
    std::string disk_index_path = std::string(get_current_dir_name()) + "/vamana_partition_disk.index";

    ASSERT_TRUE(file_exists(disk_index_path));

    std::string medoids_data_path = std::string(get_current_dir_name()) + "/vamana_partition_medoids.bin";
    ...
    load_aligned_bin(data_size, query_filename_, query, query_num, query_dim, query_aligned_dim);
    ...
    std::cout << std::setw(6) << "L" << std::setw(12) << "Beamwidth" << std::setw(16) << "QPS" << std::setw(16)
                  ...
                  << std::setw(16) << "Mean hops" << std::setw(19)  << "Mean CacheHits" << std::setw(14) << recall_string << std::endl;
    std::cout << "=========================================================================================="
                 "==========================================================================================" << std::endl;
...
#pragma omp parallel for schedule(dynamic, 1)
    for (int64_t i = 0; i < (int64_t)query_num; i++) {
        index->cached_beam_search((float *)query + (i * query_aligned_dim), recall_at, L,
    ...
    auto mean_n_cache_hits = get_mean_stats<uint32_t>(stats, query_num, [](const QueryStats &stats) { return stats.n_cache_hits; });
    ...
    std::cout << std::setw(6) << L << std::setw(12) << optimized_beamwidth << std::setw(16) << qps
                      ...
                      << std::setw(16) << mean_n_hops<< std::setw(16) << mean_n_cache_hits << std::setw(16) << recall << std::endl;
    delete[] stats;
