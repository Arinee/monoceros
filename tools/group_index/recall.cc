...
DEFINE_double(coarse_ratio, 0.1, "Coarse scan ratio");
...
public:
                   ...
                   const std::string &secondSep)
    {
        ...
        return reader.loadQuery(queryFile, firstSep, _queries);
    }
...
private:
    ...
    void recallOne(Searcher::Pointer searcher,
                   ...
                   vector<pair<fstream*, fstream*>>& outputFS)
    {
        ...
        IndexParams params;
        GeneralSearchContext knnContext(params);
        int ret = searcher->Search(query, &knnContext);
        if (ret < 0) {
        ...
        std::sort(knnRes.begin(), knnRes.end(), [](SearchResult& a, SearchResult& b) {
                                                    return a.score < b.score;
                                                });
        
        vector<SearchResult> linearRes;
        if (! _gtMode) {
            cerr << "must make groundtruth first." << endl;
            return;
        } else {
...
int main(int argc, char *argv[])
{
    ...
    index_params.set(PARAM_COARSE_SCAN_RATIO, FLAGS_coarse_ratio);
    ...
    AlgorithmFactory factory(index_params);
    ...
    // Load Index NOW
    IndexStorage::Handler::Pointer handler = stg->open(FLAGS_index, false);
    void *data = nullptr;
    ...
    if (FLAGS_type == "float") {
        ...
        recall.run(searcher, FLAGS_topk, FLAGS_gt);
