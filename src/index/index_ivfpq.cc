...
//implement indexivfpq
void IndexIvfpq::ReadPQParams()
{
...
bool IndexIvfpq::Load(IndexStorage::Handler::Pointer &&file_handle)
{
...
bool IndexIvfpq::Dump(IndexStorage::Pointer storage, const string& file_name, bool only_dump_meta)
{
    IndexPackage packageHelper;
...
bool IndexIvfpq::Create(IndexStorage::Pointer storage, const string& file_name, IndexStorage::Handler::Pointer &&meta_file_handle)
{
    ...
    if (!compIndex || !compPK || !compFeature || !compIDMap 
    ...
    if (!res || ret < 0) {
        LOG_ERROR("create ivfpq index error");
        return false;
    }
...
bool IndexIvfpq::CreatePQIndexFromPackage(map<string, size_t>& stab)
{
    ReadPQParams();
...
bool IndexIvfpq::LoadPQIndexFromPackage(IndexPackage &package)
{
...
bool IndexIvfpq::DumpPQIndexToPackage(IndexPackage &package, bool only_dump_meta, DumpContext& /*dump_context*/)
{
    // dump data
    if(!only_dump_meta)
    {
...
vector<size_t> IndexIvfpq::computeRoughLevelScanLimit(GeneralSearchContext* context)
{
...
IndexIvfpq::InitQueryDistanceMatrix(const void *query, GeneralSearchContext *context)
{
...
int IndexIvfpq::Add(docid_t doc_id, uint64_t key, const void *val, size_t len)
{
    ...
    return IndexIvfflat::Add(doc_id, key, val, len);
}
