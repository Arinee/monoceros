...
CatIvfFlatBuilder::~CatIvfFlatBuilder() {
    // free memory
    _coarseBase.reset();
    _pkBase.reset();
    _featureBase.reset();
...
int CatIvfFlatBuilder::Train(const VectorHolder::Pointer & /* holder */)
{
    return 0;
}

int CatIvfFlatBuilder::Init(const IndexMeta &meta, const IndexParams &params)
{
    _meta = meta;
    _params = params;
...
//! init from matrix
bool CatIvfFlatBuilder::InitResource(const string& rough_file)
{
...
//! Cleanup Builder
int CatIvfFlatBuilder::Cleanup(void)
{
...
int CatIvfFlatBuilder::DumpIndex(const std::string &prefix, const IndexStorage::Pointer &stg)
{
    // write left to disk
    if (_globalId > 0) {
    ...
    int64_t maxDocNum = loadSegments(fileHolder, indexSegs, pkSegs, catSegs, featureSegs);
    if (maxDocNum < 0) {
    ...
    // mmap output file
    mercury::MMapFile indexMergeFile, pkMergeFile, catMergeFile, featureMergeFile, idMapMergeFile, keyCatMapMergeFile, catSetMergeFile;
    ...
    ret = keyCatMapPtr->mount(reinterpret_cast<char *>(keyCatMapMergeFile.region()), keyCatMapMergeFile.region_size(), maxDocNum, true);
    ...
    if (!writeIndexPackage(maxDocNum, prefix, stg, indexMergeFile, pkMergeFile, 
                featureMergeFile, idMapMergeFile, keyCatMapMergeFile, catSetMergeFile)) {
...
list<Closure::Pointer> CatIvfFlatBuilder::JobSplit(const VectorHolder::Pointer &holder)
{
    ...
    for (; iter->isValid(); ) {
        ...
        job_list.push_back(Closure::New(this, &CatIvfFlatBuilder::singleTaskWithoutLabels, iter->cat(), key, data));
        iter->next();
    }
...
int32_t CatIvfFlatBuilder::getNearestLabel(shared_ptr<char> data)
{
    int return_labels  = 0;
    size_t level = 0;
    ...
    for (size_t i = 0; i < _resource->getRoughMeta().levelCnt - 1; ++i) {
        levelScanLimit.push_back(_resource->getRoughMeta().centroidNums[i]);
    }
...
void CatIvfFlatBuilder::singleTaskWithoutLabels(cat_t cat_, uint64_t key, shared_ptr<char> data)
{
    ...
    if (!doSingleBuild(cat_, key, data, roughLabel)) {
        LOG_ERROR("Failed to do single build for key[%lu]\n", key);
    }
    return;
}

bool CatIvfFlatBuilder::doSingleBuild(cat_t cat_, uint64_t key, 
...
bool CatIvfFlatBuilder::flushWithAdjust()
{
    // make dump path
...
int CatIvfFlatBuilder::initProfile(size_t elemCount, size_t elemSize)
{
    // Create CoarseIndex
    int64_t slotNum = _resource->getLeafCentroidNum();
...
size_t CatIvfFlatBuilder::memQuota2DocCount(size_t memQuota, size_t elemSize)
{
...
int64_t CatIvfFlatBuilder::loadSegments(vector<unique_ptr<mercury::MMapFile>> &fileHolder,
            vector<CoarseIndex::Pointer> &indexSegs,
            vector<ArrayProfile::Pointer> &pkSegs,
            vector<ArrayProfile::Pointer> &catSegs,
            vector<ArrayProfile::Pointer> &featureSegs)
{
    ...
    for (string segmentPath : _segmentList) {
        ...
        if (!indexFile->isValid() || !pkFile->isValid() || !catFile->isValid() || !featureFile->isValid()) {
...
bool CatIvfFlatBuilder::writeIndexPackage(size_t maxDocNum,
        const string &prefix,
        ...
        const mercury::MMapFile &catSetMergeFile)
{
    ...
    string fileName = CAT_IVFFLAT_INDEX_FILENAME;
    if (fileName.empty()) {
...
int CatIvfFlatBuilder::BuildIndex(const VectorHolder::Pointer &holder) {
    if (!holder.get()) {
        return IndexError_InvalidArgument;
    }
    ...
    thread_work_engine.Init(nullptr,
                            std::bind(&CatIvfFlatBuilder::JobSplit, this, holder));
