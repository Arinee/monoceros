...
int IvfpqBuilder::Init(const IndexMeta &meta, const IndexParams &params) {
    _meta = meta;
    _params = params;
...
//! Initialize Builder
bool IvfpqBuilder::InitResource(PQCodebook::Pointer pqCodebook) {
...
//! init from matrix
bool IvfpqBuilder::InitResource(const string &rough_file, const string &integrate_file) {
    _resource.reset(new CentroidResource);
...
int IvfpqBuilder::Train(const VectorHolder::Pointer & /* holder */)
{
    return 0;
}

bool IvfpqBuilder::IsFinish() {
    return _isRuned;
}

int IvfpqBuilder::DumpIndex(const std::string &prefix, const IndexStorage::Pointer &stg) {
    // write left to disk
    if (_globalId > 0) {
    ...
    int64_t maxDocNum = loadSegments(fileHolder, indexSegs, pkSegs, productSegs, featureSegs);
    if (maxDocNum < 0) {
    ...
    // mmap output file
    mercury::MMapFile indexMergeFile, pkMergeFile, productMergeFile, featureMergeFile, idMapMergeFile;
...
list<Closure::Pointer> IvfpqBuilder::JobSplit(const VectorHolder::Pointer &holder) {
    _isRuned = true;
    ...
    for (; iter->isValid();) {
        ...
        job_list.push_back(Closure::New(this, &IvfpqBuilder::singleTaskWithoutLabels, key, data));
        iter->next();
    }
...
void IvfpqBuilder::singleTaskWithoutLabels(uint64_t key, shared_ptr<char> data) {
    ...
    if (!doSingleBuild(key, data, roughLabel, productLabels)) {
        LOG_ERROR("Failed to do single build for key[%lu]\n", key);
    }
    return;
}

bool IvfpqBuilder::doSingleBuild(uint64_t key,
...
bool IvfpqBuilder::flushSegment() {
    // make dump path
...
bool IvfpqBuilder::flushWithAdjust() {
    // make dump path
    string segmentPath = _segmentDir + "/segment_" + to_string(_segment);
    if (!File::MakePath(segmentPath.c_str())) {
        LOG_ERROR("make segment directory[%s] failed!", segmentPath.c_str());
...
int IvfpqBuilder::initProfile(size_t elemCount, size_t elemSize) {
    // Create CoarseIndex
    int64_t slotNum = _resource->getLeafCentroidNum();
...
size_t IvfpqBuilder::memQuota2DocCount(size_t memQuota, size_t elemSize) {
...
int64_t IvfpqBuilder::loadSegments(vector<unique_ptr<mercury::MMapFile>> &fileHolder,
                                   vector<CoarseIndex::Pointer> &indexSegs,
                                   vector<ArrayProfile::Pointer> &pkSegs,
                                   vector<ArrayProfile::Pointer> &productSegs,
                                   vector<ArrayProfile::Pointer> &featureSegs) {
...
bool IvfpqBuilder::writeIndexPackage(size_t maxDocNum,
                                     const string &prefix,
...
int IvfpqBuilder::BuildIndex(const VectorHolder::Pointer &holder) {
    if (!holder.get()) {
        return IndexError_InvalidArgument;
    }
    ...
    thread_work_engine.Init(std::bind(&IvfpqBuilder::IsFinish, this),
                            std::bind(&IvfpqBuilder::JobSplit, this, holder));
