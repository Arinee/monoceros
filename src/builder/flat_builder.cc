...
int FlatBuilder::Init(const mercury::IndexMeta &meta, const mercury::IndexParams &params) {
    _meta = meta;
    _params = params;
...
int FlatBuilder::Train(const VectorHolder::Pointer & /* holder */)
{
    return 0;
}

int FlatBuilder::DumpIndex(const std::string &prefix, const IndexStorage::Pointer &stg) {
    // write left to disk
    if (_globalId > 0) {
    ...
    int64_t maxDocNum = loadSegments(fileHolder, pkSegs, featureSegs);
    if (maxDocNum < 0) {
    ...
    // mmap output file
    mercury::MMapFile pkMergeFile, featureMergeFile, idMapMergeFile;
    ...
    if (!writeIndexPackage(maxDocNum, prefix, stg, pkMergeFile, featureMergeFile, idMapMergeFile)) {
...
list<Closure::Pointer> FlatBuilder::JobSplit(const VectorHolder::Pointer &holder) {
    ...
    for (; iter->isValid();) {
        ...
        job_list.push_back(Closure::New(this, &FlatBuilder::singleTask, key, data));
        iter->next();
    }
...
void FlatBuilder::singleTask(uint64_t key, shared_ptr<char> data) {
...
bool FlatBuilder::flushWithAdjust() {
    // make dump path
...
int FlatBuilder::initProfile(size_t elemCount, size_t elemSize) {
    // Create Profiles
...
size_t FlatBuilder::memQuota2DocCount(size_t memQuota, size_t elemSize) {
...
int64_t FlatBuilder::loadSegments(vector<unique_ptr<mercury::MMapFile>> &fileHolder,
                                  vector<ArrayProfile::Pointer> &pkSegs,
                                  vector<ArrayProfile::Pointer> &featureSegs) {
    int64_t maxDocNum = 0;
    for (string segmentPath : _segmentList) {
        ...
        if (!pkFile->isValid() || !featureFile->isValid()) {
...
bool FlatBuilder::writeIndexPackage(size_t maxDocNum,
                                    const string &prefix,
    ...
    string fileName = FLAT_INDEX_FILENAME;
    if (fileName.empty()) {
...
int FlatBuilder::BuildIndex(const VectorHolder::Pointer &holder) {
    if (!holder.get()) {
        return IndexError_InvalidArgument;
    }
    ...
    thread_work_engine.Init(nullptr,
                            std::bind(&FlatBuilder::JobSplit, this, holder));
