#ifndef MERCURY_IVFFLAT_BUILDER_H__
#define MERCURY_IVFFLAT_BUILDER_H__
...
class IvfflatBuilder : public IndexBuilder
{
public:
    //! Constructor
    IvfflatBuilder() : _globalId(0), _segment(0) {}

    //! Destructor
    ~IvfflatBuilder() override;
...
private:
    bool InitResource(const std::string& rough_matrix);

    int initProfile(size_t elemCount, size_t elemSize);
    size_t memQuota2DocCount(size_t memQuota, size_t elemSize);
    void singleTaskWithoutLabels(uint64_t key, std::shared_ptr<char> data);
    ...
    int32_t getNearestLabel(std::shared_ptr<char> data);
private:
...
#endif // MERCURY_IVFFLAT_BUILDER_H__
