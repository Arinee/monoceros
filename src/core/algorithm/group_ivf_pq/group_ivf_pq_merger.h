#ifndef __MERCURY_CORE_GROUP_IVF_PQ_MERGER_H__
#define __MERCURY_CORE_GROUP_IVF_PQ_MERGER_H__
...
/*! Index Merger
 */
class GroupIvfPqMerger : public Merger
{
public:
    //! Index Merger Pointer
    typedef std::shared_ptr<GroupIvfPqMerger> Pointer;
...
private:
    ...
    int GenerateSlotsProfile(const GroupIvfPqIndex* ivf_index);

private:
    GroupIvfPqIndex merged_index_;
    std::vector<std::vector<std::vector<SlotIndex>>> indexes_slots_profile_; //index->docid->slot indexes
};

MERCURY_NAMESPACE_END(core);

#endif //__MERCURY_CORE_GROUP_IVF_PQ_MERGER_H__
