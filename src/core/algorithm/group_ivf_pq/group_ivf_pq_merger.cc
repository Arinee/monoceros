#include "group_ivf_pq_merger.h"

MERCURY_NAMESPACE_BEGIN(core);

//! Initialize Merger
int GroupIvfPqMerger::Init(const IndexParams &params) {
    merged_index_.SetIndexParams(params);
    return 0;
}

template <typename T>
int GroupIvfPqMerger::DoMergeEachIvf(docid_t new_id, T old_id_pk, size_t index_offset,
                              const std::vector<Index::Pointer> &indexes, bool by_pk,
                              GroupIvfPqIndex* merged_index) {
    ...
    auto ivf_index = dynamic_cast<GroupIvfPqIndex*>(indexes.at(index_offset).get());
    if (by_pk) {
        if (!ivf_index->WithPk()) {
...
template <typename T>
int GroupIvfPqMerger::DoMerge(const std::vector<Index::Pointer> &indexes,
    ...
    for (const auto& index : indexes) {
        const GroupIvfPqIndex* ivf_index = dynamic_cast<GroupIvfPqIndex*>(index.get());
    ...
    auto front_index = dynamic_cast<GroupIvfPqIndex*>(indexes.front().get());
...
int GroupIvfPqMerger::GenerateSlotsProfile(const GroupIvfPqIndex* ivf_index) {
...
int GroupIvfPqMerger::MergeByDocid(const std::vector<Index::Pointer> &indexes, const MergeIdMap& id_map) {
    return DoMerge(indexes, id_map, false);
}

int GroupIvfPqMerger::MergeByPk(const std::vector<Index::Pointer> &indexes, const MergePkMap& pk_map) {
    return DoMerge(indexes, pk_map, true);
}

//! Dump index into file or memory
int GroupIvfPqMerger::DumpIndex(const std::string &path, const IndexStorage::Pointer &stg) {
    return -1;
}

const void * GroupIvfPqMerger::DumpIndex(size_t *size) {
...
bool GroupIvfPqMerger::CheckSize(const std::vector<Index::Pointer> &indexes, size_t new_size) const {
    // check
    size_t total_size = 0;
