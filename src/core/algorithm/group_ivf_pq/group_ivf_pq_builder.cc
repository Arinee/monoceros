#include "group_ivf_pq_builder.h"
...
//! Initialize Builder
int GroupIvfPqBuilder::Init(IndexParams &params)
{
    if (!index_) {
...
//! Build the index
int GroupIvfPqBuilder::AddDoc(docid_t doc_id, uint64_t pk, const std::string &build_str, const std::string &primary_key)
{
    return index_->Add(doc_id, pk, build_str, primary_key);
}

int GroupIvfPqBuilder::GetRankScore(const std::string &build_str, float *score)
{
...
float GroupIvfPqBuilder::CalcScore(uint32_t level, SlotIndex label)
{
    return level == sort_build_group_level_ ? (label + 1) : 0;
}

//! Dump index into file or memory
int GroupIvfPqBuilder::DumpIndex(const std::string &path, const IndexStorage::Pointer &stg)
{
    // TODO
    return 0;
}

const void *GroupIvfPqBuilder::DumpIndex(size_t *size)
{
