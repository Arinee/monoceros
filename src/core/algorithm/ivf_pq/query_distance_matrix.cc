...
bool QueryDistanceMatrix::init(const void *queryFeature, const std::vector<size_t>& levelScanLimit, bool withCodeFeature)
{
...
bool QueryDistanceMatrix::initDistanceMatrix(const void *queryFeature, bool withCodeFeature)
{
    _withCodeFeature = withCodeFeature;
    if (_fragmentIndexMeta.sizeofElement() != _elemSize) {
        std:: cerr << "index meta not match with centroid resource, element size: "
...
bool QueryDistanceMatrix::computeCentroid(const void *feature, const std::vector<size_t>& levelScanLimit)
{
...
bool QueryDistanceMatrix::computeDistanceMatrix(const void *feature)
{
    if (UNLIKELY(_withCodeFeature)) {
        _queryCodeFeature = new(std::nothrow) uint16_t[_fragmentNum];
