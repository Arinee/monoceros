#include <math.h>
#include "ivf_fast_scan_builder.h"
#include "src/core/algorithm/query_info.h"

MERCURY_NAMESPACE_BEGIN(core);

//! Initialize Builder
int IvfFastScanBuilder::Init(IndexParams &params) {
    if (!index_) {
...
//! Build the index
int IvfFastScanBuilder::AddDoc(docid_t doc_id, uint64_t pk,
                              const std::string& build_str, 
                              const std::string& primary_key) {
    return index_->Add(doc_id, pk, build_str, primary_key);
}

int IvfFastScanBuilder::GetRankScore(const std::string& build_str, float * score) {
...
float IvfFastScanBuilder::CalcScore(uint32_t level, SlotIndex label) {
    return level == sort_build_group_level_ ? (label + 1) : 0;
}

//! Dump index into file or memory
int IvfFastScanBuilder::DumpIndex(const std::string &path, const IndexStorage::Pointer &stg) {
    //TODO
    return 0;
}

const void * IvfFastScanBuilder::DumpIndex(size_t* size) {
