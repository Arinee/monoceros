 ...
 *   \file     fastscan_kernel.h
 *   \author   haiming.sy
 *   \date     Aug 2024
 *   \version  1.0.0
 *   \brief    Interface of Monoceros FastScan (internal)
 */
#pragma once
...
#include "src/core/framework/utility/internal/cpu_features.h"
...
struct simd16uint16 : simd256bit {
    ...
    // Print function using printf
    void print() const {
        // Extract the 32 bytes into an array
        alignas(32) uint16_t values[16];
        _mm256_store_si256(reinterpret_cast<__m256i*>(values), i);

        // Print each 2-bytes as an unsigned integer
        for (int j = 0; j < 16; ++j) {
            printf("%hu ", values[j]);
        }
        printf("\n");
