...
MatrixPointer IvfFastScanIndex::NullMatrix() const {
    return MatrixPointer(nullptr);
}

IvfFastScanIndex::IvfFastScanIndex() {
    SetThreadEnv();
}

int IvfFastScanIndex::Load(const void* data, size_t size) {
    LOG_INFO("Begin to Load ivf_fast_scan_index. size: %lu.", size);
    ...
    size_t centroid_num = GetPqCentroidResource().getIntegrateMeta().centroidNum;
    if (centroid_num != 16) {
        LOG_ERROR("Failed to load index, Invalid centroid number %lu for fastscan", centroid_num);
        return -1;
    }

    component = index_package_.get(COMPONENT_PQ_CODE_PROFILE);
    if (!component) {
        LOG_INFO("PQ index not exist, load fastscan index for search");
        auto aligned_component = index_package_.getAligned32Seg();
        if (!aligned_component) {
            LOG_ERROR("get aligned_component failed");
            return -1;
        }
        if (!fastscan_index_.load(aligned_component, index_package_.getAligned32Size())) {
            LOG_ERROR("fastscan index load error");
            return -1;
        }
    } else {
        LOG_INFO("Load PQ index for merge");
...
bool IvfFastScanIndex::InitFastScanIndex(const std::vector<size_t>& codeCounts) {
    auto slot_num = centroid_resource_manager_.GetTotalCentroidsNum();
    auto capacity = FastScanIndex::calcSize(slot_num, codeCounts);
    if (fastscan_base_ != nullptr) {
        free(fastscan_base_);
    }
    fastscan_base_ = aligned_alloc(32, capacity);
    if (!fastscan_index_.create(fastscan_base_, slot_num, codeCounts)) {
        LOG_ERROR("Failed to create FastScanIndex.");
        return false;
    }
    return true;
}

int IvfFastScanIndex::CopyInit(const Index* index, size_t doc_num) {
    if (!index) {
    ...
    const IvfFastScanIndex* ivf_index = dynamic_cast<const IvfFastScanIndex*>(index);
    ...
    if (MultiAgeMode()) {
        ...
        if (!doc_create_time_profile_.create(doc_create_time_profile_base_.data(),
                                        capacity, elem_size)) {
...
int IvfFastScanIndex::Create(IndexParams& index_params) {
...
MatrixPointer IvfFastScanIndex::DoLoadCentroidMatrix(const std::string &file_path, size_t dimension, size_t element_size,
...
bool IvfFastScanIndex::StrToValue(const std::string& source, void* value) const {
...
bool IvfFastScanIndex::InitIvfCentroidMatrix(const std::string& centroid_dir) {
...
bool IvfFastScanIndex::InitPqCentroidMatrix(const IndexParams& param) {
    ...
    if (pq_centroid_num != 16) {
        LOG_ERROR("FastScan only support 16 centroid number for pq.");
        return false;
    }
...
bool IvfFastScanIndex::ResolveGroupFile(const std::string& meta_path, std::vector<GroupInfo>& groups,
                      std::unordered_map<GroupInfo, uint32_t, GroupHash, GroupCmp>& group_centroids) {
    //TODO, resolve group
    std::string file_content;
...
/// add a new vector
int IvfFastScanIndex::Add(docid_t doc_id, pk_t pk,
    ...
    for (size_t i = 0; i < group_infos.size(); i++) {
        ...
        SlotIndex label = GetNearestGroupLabel(query_info.GetVector(), query_info.GetVectorLen(),
                                                group_index, centroid_resource_manager_);
        ...
        if (i == 0) {
            LoopUpTable qdm(index_meta_, &GetPqCentroidResource());
            ...
            bool bres = qdm.init(query_info.GetVector(), level_scan_limit, true);
            if (!bres) {
...
SlotIndex IvfFastScanIndex::GetNearestGroupLabel(const void* data, size_t /* size */, gindex_t group_index,
...
bool IvfFastScanIndex::IsFull() const {
    //here current doc num从coarseIndex里面拿
    return GetDocNum() >= GetMaxDocNum();
}

// rank score in merger
float IvfFastScanIndex::GetRankScore(docid_t doc_id) {
    return *(SlotIndex*)rank_score_profile_.getInfo(doc_id);
}

//计算给定内存下能够存储多少文档
size_t IvfFastScanIndex::MemQuota2DocCount(size_t memQuota, size_t /* elemSize */) const {
    ...
    if (elemCount == 0) {
        ...
        std::cerr << "slotNum:" << slotNum << ", feature size: " << GetFeatureSize() << ", product size: "<< product_size << std::endl;
...
int IvfFastScanIndex::Dump(const void*& data, size_t& size) {
    ...
    index_package_.emplace(COMPONENT_COARSE_INDEX, coarse_index_.GetBasePtr(),
                           coarse_index_.getHeader()->capacity);

    if (fastscan_base_ == nullptr) {
        LOG_INFO("FastScan Index is not created, dump PQ index for merge");
        index_package_.emplace(COMPONENT_PQ_CODE_PROFILE, pq_code_profile_.getHeader(),
                               pq_code_profile_.getHeader()->capacity);
    } else {
        LOG_INFO("FastScan Index is created, dump fastscan index for search");
        index_package_.emplace(COMPONENT_FASTSCAN_INDEX, fastscan_index_.GetBasePtr(),
                                fastscan_index_.getHeader()->capacity);
    }
...
int64_t IvfFastScanIndex::UsedMemoryInCurrent() const {
    int64_t result = sizeof(*this) + coarse_index_.getHeader()->usedSize + pq_code_profile_.getHeader()->capacity + rank_score_profile_.getHeader()->capacity;
    if (ContainFeature()) {
...
bool IvfFastScanIndex::NeedParralel(size_t centroid_num) const {
    if (!mercury_need_parallel
        || centroid_num <= (size_t)mercury_min_parralel_centroids) {
        return false;
    }
...
int IvfFastScanIndex::PushQueue(const std::vector<CentroidInfo>& infos, CentroidQueue& cq) const {
...
void* IvfFastScanIndex::BthreadRun(void* message) {
    BthreadMessage* msg = static_cast<BthreadMessage*>(message);
    if (msg->index) {
        msg->index->CalcCentroidScore(*msg->centroid_resource, 
                                      msg->start, msg->end, 
                                      msg->centroid_infos, 
                                      msg->datas, 
                                      msg->query_dimension);
...
int IvfFastScanIndex::CalcCentroidScore(CentroidResource& centroid_resource, size_t start, size_t end,
...
int IvfFastScanIndex::CalcFineCentroidScore(CentroidResource &centroid_resource, size_t start, size_t end,
...
int IvfFastScanIndex::SearchIvf(gindex_t group_index, const void* datas,
                               size_t size, size_t query_dimension,
    ...
    float downgrade_percent = context_params.has(PARAM_DOWNGRADE_PERCENT) ? context_params.getFloat(PARAM_DOWNGRADE_PERCENT) : 1;
...
void IvfFastScanIndex::GetPostNPackerFromSlot(std::vector<CoarseIndex<BigBlock>::PostingIterator> &postings,
                                            ...
                                            bool is_multi_query)
{
    ...
    const FastScanIndex& fastscan_index = GetFastScanIndex();
    for (size_t i = 0; i < real_slot_indexs.size(); ++i) {
        postings.reserve(postings.size() + real_slot_indexs[i].size());
        code_packers.reserve(code_packers.size() + real_slot_indexs[i].size());
        for (auto &group_real_slot_index : real_slot_indexs[i]) {
            ...
            FastScanIndex::PackedCodeIterator codePacker = fastscan_index.search(group_real_slot_index);
            group_doc_nums[i] += posting.getDocNum();
            postings.push_back(posting);
            code_packers.push_back(codePacker);
