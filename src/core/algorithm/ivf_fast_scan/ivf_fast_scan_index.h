...
class IvfFastScanIndex : public Index {
public:
    typedef std::shared_ptr<IvfFastScanIndex> Pointer;
    IvfFastScanIndex();
    /// load index from index package file and index will save this handle
    int Load(const void*, size_t) override;
    /// add a new vectoV
    int Add(docid_t doc_id, pk_t pk,
    ...
    bool InitFastScanIndex(const std::vector<size_t>&);
...
public:
    ...
    const CoarseIndex<BigBlock>& GetCoarseIndex() const {
        return coarse_index_;
    }

    FastScanIndex& GetFastScanIndex() {
        return fastscan_index_;
    }

    const FastScanIndex& GetFastScanIndex() const {
        return fastscan_index_;
    }
    ...
    int SearchIvf(gindex_t group_index, const void* datas, size_t size,
                  size_t query_dimension, const IndexParams& context_params,
                  std::vector<off_t>& real_slot_indexs);

    void GetPostNPackerFromSlot(std::vector<CoarseIndex<BigBlock>::PostingIterator> &postings,
...
private:
    ...
    CoarseIndex<BigBlock> coarse_index_;  //改为大block
    std::vector<char> coarse_base_;
    FastScanIndex fastscan_index_; // for 4-bits pq fastscan
    void* fastscan_base_ = nullptr;
