...
#include "src/core/algorithm/gpu_group_ivf_pq_small/gpu_group_ivf_pq_small_searcher.h"
...
GpuGroupIvfPqSmallSearcher::GpuGroupIvfPqSmallSearcher()
{
    if (!index_) {
...
int GpuGroupIvfPqSmallSearcher::Init(IndexParams &params)
{
    index_->SetIndexParams(params);
    index_->SetIsGpu(true);

    std::string index_name = params.getString(PARAM_VECTOR_INDEX_NAME);
    LOG_INFO("Start Init GpuGroupIvfPqSmallSearcher, %s", index_name.c_str());
...
int GpuGroupIvfPqSmallSearcher::LoadIndex(const std::string &path)
{
    // TODO
    return -1;
}

int GpuGroupIvfPqSmallSearcher::LoadIndex(const void *data, size_t size)
{
...
void GpuGroupIvfPqSmallSearcher::SetIndex(Index::Pointer index)
{
    index_ = std::dynamic_pointer_cast<GroupIvfPqSmallIndex>(index);
    dist_scorer_factory_.Init(index_->GetIndexMeta());
}

void GpuGroupIvfPqSmallSearcher::SetBaseDocId(exdocid_t baseDocId)
{
    base_docid_ = baseDocId;
    index_->SetBaseDocid(baseDocId);
}

IndexMeta::FeatureTypes GpuGroupIvfPqSmallSearcher::getFType()
{
    return this->index_->GetIndexMeta().type();
}

//! search by query
int GpuGroupIvfPqSmallSearcher::Search(const QueryInfo &query_info, GeneralSearchContext *context)
{
...
int GpuGroupIvfPqSmallSearcher::CollectIvfPostings(std::vector<CoarseIndex<SmallBlock>::PostingIterator> &ivf_postings,
                                              std::vector<uint32_t> &ivf_postings_group_ids,
...
void GpuGroupIvfPqSmallSearcher::RecoverPostingFromSlot(std::vector<CoarseIndex<SmallBlock>::PostingIterator> &postings,
                                                   std::vector<uint32_t> &ivf_postings_group_ids,
                                                   std::vector<std::vector<off_t>> &real_slot_indexs,
...
int GpuGroupIvfPqSmallSearcher::CollectIvfDistNodes(const QueryInfo &query_info, std::vector<uint32_t> &dist_nodes,
                                               std::vector<CoarseIndex<SmallBlock>::PostingIterator> &ivf_postings,
...
int GpuGroupIvfPqSmallSearcher::PostProcess(const QueryInfo &query_info, GeneralSearchContext *context,
