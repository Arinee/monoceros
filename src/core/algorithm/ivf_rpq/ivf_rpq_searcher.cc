...
void* IvfRpqSearcher::BthreadRun(void* message) {
...
void IvfRpqSearcher::BatchScore(size_t node_start, size_t node_end,
...
void IvfRpqSearcher::BatchScoreRpq(size_t node_start, size_t node_end,
                                      std::vector<DistNode>* dist_nodes1,
                                      QueryDistanceMatrix1* qdm) const {
    PqDistScorer1 scorer(&index_->GetPqCodeProfile());
    for (size_t i = node_start; i < node_end; i++) {
        DistNode& dist_node = dist_nodes1->at(i);
        // load term2: || y_R ||^2 + 2 * (C|y_R) = (y_R|y_R) + 2 * (C|y_R)
        const distance_t *ipCentArray = reinterpret_cast<const distance_t *>(index_->GetCentRefineProfile().getInfo(dist_node.offset));
        dist_node.dist += scorer.score(dist_node.key, qdm, ipCentArray);
    }
}

size_t IvfRpqSearcher::GetPostingsNodeCount(const std::vector<CoarseIndex<SmallBlock>::PostingIterator>& ivf_postings,
...
int IvfRpqSearcher::Init(IndexParams &params) {
    index_->SetIndexParams(params);

    std::string index_name = params.getString(PARAM_VECTOR_INDEX_NAME);
    LOG_INFO("Start Init IvfRpqSearcher, %s", index_name.c_str());

    MONITOR_TRANSACTION(IvfRpq, CollectIvfPostings);
    MONITOR_TRANSACTION(IvfRpq, CalcNodeDist);
    MONITOR_TRANSACTION(IvfRpq, CollectGroupHeaps);
    MONITOR_TRANSACTION(IvfRpq, CollectBasicResult);
    MONITOR_TRANSACTION(IvfRpq, CollectLeftResult);
    MONITOR_TRANSACTION(IvfRpq, CollectNthTopk);
    MONITOR_TRANSACTION(IvfRpq, PostProcess);
    MONITOR_TRANSACTION(IvfRpq, CollectMultiAgeResult);
    MONITOR_TRANSACTION(IvfRpq, StatAgeInfo);
    MONITOR_TRANSACTION(IvfRpq, GenerateAgeSortedContainer);
    MONITOR_TRANSACTION(IvfRpq, SortInEachAge);
    MONITOR_METRIC_WITH_INDEX(IvfRpq_CentroidNum, "IvfRpq_CentroidNum_" + index_name);
    MONITOR_METRIC_WITH_INDEX(IvfRpq_GroupNum, "IvfRpq_GroupNum_" + index_name);
    MONITOR_METRIC_WITH_INDEX(IvfRpq_FullDocNum, "IvfRpq_FullDocNum_" + index_name);
    MONITOR_METRIC_WITH_INDEX(IvfRpq_RtDocNum, "IvfRpq_RtDocNum_" + index_name);

    return 0;
}

int IvfRpqSearcher::LoadIndex(const std::string& path) {
    //TODO
    return -1;
}

int IvfRpqSearcher::LoadIndex(const void* data, size_t size) {
    if (index_->Load(data, size) != 0) {
        LOG_ERROR("Failed to load index.");
        return -1;
    }
    if (index_->EnableQuantize()) {
        LOG_INFO("Vmin = %f and Vmax = %f", index_->GetVmin(), index_->GetVmax());
    }
    dist_scorer_factory_.Init(index_->GetIndexMeta());
    LOG_INFO("Level1 centroids number is %lu", index_->GetCentroidResourceManager().GetTotalCentroidsNum());
    return 0;
}

void IvfRpqSearcher::SetIndex(Index::Pointer index)
{
    index_ = std::dynamic_pointer_cast<IvfRpqIndex>(index);
    dist_scorer_factory_.Init(index_->GetIndexMeta());
}

void IvfRpqSearcher::SetBaseDocId(exdocid_t baseDocId)
{
    base_docid_ = baseDocId;
    index_->SetBaseDocid(baseDocId);
}

IndexMeta::FeatureTypes IvfRpqSearcher::getFType()
{
    return this->index_->GetIndexMeta().type();
}

int IvfRpqSearcher::CollectMultiAgeResult(const QueryInfo& query_info,
                                              std::vector<DistNode>& dist_nodes,
...
int IvfRpqSearcher::CollectNthTopk(uint32_t start, uint32_t end, uint32_t topk,
                                       std::vector<DistNode>& dist_nodes,
...
int IvfRpqSearcher::Search(const QueryInfo& query_info, GeneralSearchContext* context) {
    ...
    MONITOR_TRANSACTION_LOG(CollectIvfPostings(ivf_postings, ivf_postings_group_ids, 
                            group_doc_nums, query_info, real_slot_indexs, lvl1_idx_dis), 
            "collect ivf posting failed.",
            IvfRpq,
            CollectIvfPostings)

    MONITOR_METRIC_LOG(IvfRpq_CentroidNum, ivf_postings.size());
    MONITOR_METRIC_LOG(IvfRpq_GroupNum, group_infos.size());

    std::vector<DistNode> dist_nodes;

    // optimized approach
    MONITOR_TRANSACTION_LOG(CalcNodeDist(ivf_postings, query_info, dist_nodes, lvl1_idx_dis), 
...
int IvfRpqSearcher::CalcNodeDist(std::vector<CoarseIndex<SmallBlock>::PostingIterator>& ivf_postings,
    ...
    if (base_docid_ == 0) {
        MONITOR_METRIC_LOG(IvfRpq_FullDocNum, node_count);
    } else {
        MONITOR_METRIC_LOG(IvfRpq_RtDocNum, node_count);
    }
    ...
    if (is_use_pq) {
        ...
        QueryDistanceMatrix1* qdm = new QueryDistanceMatrix1(index_->GetIndexMeta(), &(index_->GetPqCentroidResource()));
        ...
        for (size_t i = 0; i < ivf_postings.size(); i++) {
            if (ivf_postings[i].getDocNum() != 0) {
                ...
                while (UNLIKELY(!iter.finish())) {
                    ...
                    dist_nodes.emplace_back(docid, lvl1_idx_dis[i].second, lvl1_idx_dis[i].first);
                }
            }
        }
        BatchScoreRpq(node_start_, node_end_, &dist_nodes, qdm);
        delete qdm;
        return 0;
    }
    ...
    for (size_t i = 0; i < ivf_postings.size(); i++) {
        ...
        while (UNLIKELY(!iter.finish())) {
            docid_t docid = iter.next();
            dist_nodes.emplace_back(docid, 0, 0);
        }
    }
...
int IvfRpqSearcher::CollectIvfPostings(std::vector<CoarseIndex<SmallBlock>::PostingIterator>& ivf_postings,
                                          std::vector<uint32_t>& ivf_postings_group_ids,
    ...
    if (real_slot_indexs.empty()) {
        ...
        for (size_t i = 0; i < group_infos.size(); i++) {
            ...
            if (index_->SearchIvf(group_index, query_info.MultiQueryMode() ? query_info.GetVectors().at(i) : query_info.GetVectors().at(0),
                                  ...
                                  query_info.GetContextParams(), real_slot_indexs[i], lvl1_idx_dis) != 0) {
...
int IvfRpqSearcher::CollectGroupHeaps(const QueryInfo& query_info,
                                          std::vector<DistNode>& dist_nodes,
...
int IvfRpqSearcher::CollectBasicResult(std::vector<MyHeap<DistNode>>& group_heaps, const std::vector<uint32_t>& topks,
                                        GeneralSearchContext* context) const {
...
int IvfRpqSearcher::CollectLeftResult(uint32_t total, std::vector<MyHeap<DistNode>>& group_heaps,
                                        GeneralSearchContext* context) const {
...
int IvfRpqSearcher::PostProcess(const QueryInfo& query_info, GeneralSearchContext* context, 
...
void IvfRpqSearcher::PushSearchResultToContext(GeneralSearchContext* context, const std::vector<DistNode>& dist_vec) const {
    bool with_pk = index_->WithPk();
