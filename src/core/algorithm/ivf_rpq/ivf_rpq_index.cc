...
#include "src/core/algorithm/ivf_rpq/query_distance_matrix1.h"
...
MatrixPointer IvfRpqIndex::NullMatrix() const
{
    return MatrixPointer(nullptr);
}

IvfRpqIndex::IvfRpqIndex()
{
    SetThreadEnv();
}

int IvfRpqIndex::Load(const void *data, size_t size)
{
    ...
    if (EnableQuantize()) {
        component = index_package_.get(COMPONENT_VMIN);
        if (!component) {
            LOG_ERROR("get component error: %s", COMPONENT_VMIN);
            return -1;
        }
        SetVmin(*((float *)component->getData()));

        component = index_package_.get(COMPONENT_VMAX);
        if (!component) {
            LOG_ERROR("get component error: %s", COMPONENT_VMAX);
            return -1;
        }
        SetVmax(*((float *)component->getData()));   
    }
    ...
    if (!coarse_index_.load((void *)component->getData(), component->getDataSize())) {
        ...
        LOG_INFO("coarse index load success with size of %lu", component->getDataSize());
    }
    ...
    component = index_package_.get(COMPONENT_CENT_REFINE_PROFILE);
    if (component) {
        if (!cent_refine_profile_.load((void *)component->getData(), component->getDataSize())) {
            LOG_ERROR("cent_refine_profile load error");
            return -1;
        } else {
            LOG_INFO("cent_refine_profile load success with size of %lu", component->getDataSize());
        }
    } else {
        LOG_INFO("Init cent refine profile in merge stage");
    }
    ...
    if (ContainFeature()) {
        ...
        if (!component) {
            ...
            if (!feature_profile_.load((void *)component->getData(), component->getDataSize())) {
                ...
                LOG_INFO("feature profile load success with size of %lu", component->getDataSize());
            }
        }
    }
    ...
    if (!component) {
        ...
        if (!pq_code_profile_.load((void *)component->getData(), component->getDataSize())) {
            ...
            LOG_INFO("PQ code profile load success with size of %lu", component->getDataSize());
        }
    }
    ...
    if (!component) {
        ...
        if (!rank_score_profile_.load((void *)component->getData(), component->getDataSize())) {
            ...
            LOG_INFO("Rank score profile load success with size of %lu", component->getDataSize());
        }
    }
...
int IvfRpqIndex::CopyInit(const Index *index, size_t doc_num)
{
    if (!index) {
    ...
    const IvfRpqIndex *ivf_index = dynamic_cast<const IvfRpqIndex *>(index);
    ...
    size_t lvl1_cent_num = centroid_resource_manager_.GetTotalCentroidsNum();
    auto pq_frag_num = GetPqCentroidResource().getIntegrateMeta().fragmentNum;
    auto pq_cent_num = GetPqCentroidResource().getIntegrateMeta().centroidNum;
    size_t frag_dim = index_meta_.dimension() / pq_frag_num;
    size_t cent_refine_num = pq_frag_num * pq_cent_num;
    size_t cent_refine_capacity = ArrayProfile::CalcSize(lvl1_cent_num, cent_refine_num * sizeof(distance_t));
    cent_refine_base_.assign(cent_refine_capacity, 0);
    if (!cent_refine_profile_.create(cent_refine_base_.data(), cent_refine_capacity, cent_refine_num * sizeof(distance_t))) {
        LOG_ERROR("Failed to create cent refine profile");
        return -1;
    }

    IndexMeta fragmentIndexMeta(index_meta_);
    fragmentIndexMeta.setDimension(frag_dim);

    auto elem_size = centroid_resource_manager_.GetCentroidResource(0).getIntegrateMeta().elemSize;

    // calc term2: || y_R ||^2 + 2 * (C|y_R) = (y_R|y_R) + 2 * (C|y_R)
    std::vector<distance_t> cent_ipVals;
    cent_ipVals.reserve(cent_refine_num);

    for (size_t i = 0; i < lvl1_cent_num; i++) {
        auto centroid_value = centroid_resource_manager_.GetCentroidResource(0).getValueInRoughMatrix(0, i);

        for (size_t fragmentIndex = 0; fragmentIndex < pq_frag_num ; ++fragmentIndex) {
            const char* fragmentFeature = reinterpret_cast<const char*>(centroid_value) + fragmentIndex * elem_size;
            for (size_t centroidIndex = 0; centroidIndex < pq_cent_num; ++centroidIndex) {
                const void* fragmentCenter = centroid_resource_manager_.GetCentroidResource(0).getValueInIntegrateMatrix(fragmentIndex, centroidIndex);
                size_t index = fragmentIndex * pq_cent_num + centroidIndex;
                distance_t cent_ipVal = - fragmentIndexMeta.fip(fragmentCenter, fragmentCenter) 
                                        - 2 * fragmentIndexMeta.fip(fragmentFeature, fragmentCenter);
                cent_ipVals.push_back(cent_ipVal);
            }
        }

        cent_refine_profile_.insert(i, cent_ipVals.data());
    ...
    if (ContainFeature()) {
        ...
        if (EnableQuantize()) {
            elem_size = sizeof(uint8_t) * index_meta_.dimension();
        }
        LOG_INFO("elem_size for merged index is %lu", elem_size);
    ...
    size_t product_size = sizeof(uint8_t) * fragment_num;
...
int IvfRpqIndex::Create(IndexParams &index_params)
{
    ...
    if (EnableQuantize() && !ContainFeature()) {
        LOG_ERROR("Quantize requires feature storage!");
        return -1;
    }

    if (EnableQuantize() && index_meta_.type() != IndexMeta::FeatureTypes::kTypeFloat) {
        LOG_ERROR("Quantize only supported for float type!");
        return -1;
    }
    ...
    if (group_manager_.GetGroupNum() != 1) {
    ...
    auto product_size = sizeof(uint8_t) * fragment_num;
...
MatrixPointer IvfRpqIndex::DoLoadCentroidMatrix(const std::string &file_path, size_t dimension, size_t element_size,
...
bool IvfRpqIndex::StrToValue(const std::string &source, void *value) const
{
...
bool IvfRpqIndex::InitIvfCentroidMatrix(const std::string &centroid_dir)
{
...
bool IvfRpqIndex::InitPqCentroidMatrix(const IndexParams &param)
{
...
bool IvfRpqIndex::ResolveGroupFile(const std::string &meta_path, std::vector<GroupInfo> &groups,
                                       std::unordered_map<GroupInfo, uint32_t, GroupHash, GroupCmp> &group_centroids)
{
    // TODO, resolve group
    std::string file_content;
...
/// add a new vector
int IvfRpqIndex::Add(docid_t doc_id, pk_t pk, const std::string &query_str, const std::string &primary_key)
{
    ...
    if (EnableQuantize()) {
        for (size_t i = 0; i < index_meta_.dimension(); i++) {
            float val = (float)*((float *)(query_info.GetVector()) + i);
            if (val > GetVmax()) {
                SetVmax(val);
            }
            if (val < GetVmin()) {
    ...
    for (size_t i = 0; i < group_infos.size(); i++) {
        ...
        if (i == 0) {
            QueryDistanceMatrix1 qdm(index_meta_, &GetPqCentroidResource());
            ...
            std::vector<uint8_t> product_labels;
...
SlotIndex IvfRpqIndex::GetNearestGroupLabel(const void *data, size_t /* size */, gindex_t group_index,
...
float IvfRpqIndex::GetRankScore(docid_t doc_id)
{
    return *(SlotIndex *)rank_score_profile_.getInfo(doc_id);
}

// 计算给定内存下能够存储多少文档
size_t IvfRpqIndex::MemQuota2DocCount(size_t memQuota, size_t /* elemSize */) const
{
...
int IvfRpqIndex::Dump(const void *&data, size_t &size)
{
    ...
    if (EnableQuantize()) {
        LOG_INFO("Vmin = %f and Vmax = %f", GetVmin(), GetVmax());
        float vmin = GetVmin();
        float vmax = GetVmax();
        index_package_.emplace(COMPONENT_VMIN, &vmin, sizeof(vmin));
        index_package_.emplace(COMPONENT_VMAX, &vmax, sizeof(vmax));
    }
    ...
    if (!cent_refine_base_.empty()) {
        LOG_INFO("cent_refine_base_ is not empty, then dump it");
        index_package_.emplace(COMPONENT_CENT_REFINE_PROFILE, cent_refine_profile_.getHeader(),
                               cent_refine_profile_.getHeader()->capacity);
    }
...
int64_t IvfRpqIndex::UsedMemoryInCurrent() const
{
...
bool IvfRpqIndex::NeedParralel(size_t centroid_num) const
{
...
int IvfRpqIndex::PushQueue(const std::vector<CentroidInfo> &infos, CentroidQueue &cq) const
{
...
void *IvfRpqIndex::BthreadRun(void *message)
{
...
int IvfRpqIndex::CalcCentroidScore(CentroidResource &centroid_resource, size_t start, size_t end,
...
int IvfRpqIndex::CalcFineCentroidScore(CentroidResource &centroid_resource, size_t start, size_t end,
...
int IvfRpqIndex::SearchIvf(gindex_t group_index, const void *datas, size_t size, size_t query_dimension,
                               ...
                               std::vector<std::pair<uint32_t, distance_t>>& lvl1_idx_dis)
{
    ...
    std::unordered_map<uint32_t, distance_t> tmp_lvl1_idx_dis;

    group_real_slot_indexs.reserve(nprobe);
    if (!EnableFineCluster()) {
        lvl1_idx_dis.reserve(nprobe);
    }
    size_t i = 0;
    while (!centroid_resource_s.empty() && i++ < nprobe) {
        auto index_label = centroid_resource_s.top().index;
        auto distance = centroid_resource_s.top().dist;
        if (!EnableFineCluster()) {
            lvl1_idx_dis.push_back(std::make_pair(index_label, distance));
        } else {
            tmp_lvl1_idx_dis[index_label] = distance;
        }
    ...
    if (EnableFineCluster()) {
        ...
        lvl1_idx_dis.reserve(fine_nprobe);
        for (size_t i = 0; i < fine_nprobe; i++) {
            ...
            lvl1_idx_dis.push_back(std::make_pair(fine_centroid_infos[i].coarse_index, tmp_lvl1_idx_dis[fine_centroid_infos[i].coarse_index]));
        }
    ...
    std::sort(lvl1_idx_dis.begin(), lvl1_idx_dis.end());

    return 0;
}

void IvfRpqIndex::RecoverPostingFromSlot(std::vector<CoarseIndex<SmallBlock>::PostingIterator> &postings,
...
int IvfRpqIndex::SearchGroup(docid_t docid, std::vector<GroupInfo> &group_infos, std::vector<int> &labels) const
{
