...
bool CentroidResource::initIntegrate(void *base, size_t len)
{
    _roughOnly = false;
    _memAlloc = false;
    if (!parseIntegrateContent((char *)base, len)) {
        return false;
    }
    return validate();
}
...
bool CentroidResource::create(const IntegrateMeta& integrateMeta) {
    _roughOnly = false;
...
{
    return create(roughMeta) && create(integrateMeta);
}
...
bool CentroidResource::parseRoughContent(char *pBase, size_t fileLength)
{
    if (fileLength < 4 * sizeof(uint32_t)) {
        std::cerr << "rough matrix content error length: " << fileLength << std::endl;
        return false;
    }
    char* buf = pBase;
    size_t offset = 0;
    ...
    {
        std::cerr << "rough matrix length is not equal rightlength" << std::endl;
        return false;
    }
...
{
    if (fileLength < 4 * sizeof(uint32_t)) {
        std::cerr << "integrate matrix content error length" << std::endl;
        return false;
    }
    char* buf = pBase;
    size_t offset = 0;
    ...
    if (fileLength != rightFileLength)
    {
        std::cerr << "integrate matrix length[] is not equal rightlength[]" << std::endl;
        return false;
    }
...
bool CentroidResource::DumpToFile(const string rough_name)
{
    ...
    if(!rough_name.empty())
    {
        ...
        if (cnt != tmp.size()) {
            std::cerr << "Write file header to file [] fail" << std::endl;
            fclose(fp);
            return false;
        }
        fclose(fp);
