#ifndef __MERCURY_CORE_GROUP_IVF_INDEX_H__
#define __MERCURY_CORE_GROUP_IVF_INDEX_H__
...
class GroupIvfIndex : public Index
{
public:
    typedef std::shared_ptr<GroupIvfIndex> Pointer;
    GroupIvfIndex();
    ...
    {
        return *(float *)rank_score_profile_.getInfo(doc_id);
    };
    ...
    int DumpCentroid(const void *&data, size_t &size) override;
...
protected:
    ...
    bool InitMipsTranform(const std::string &centroid_dir);
...
private:
    ...
    float BestDistance(const std::vector<const void *> &datas, size_t query_dimension, const void *centroid,
                       OrigDistScorer scorer) const;
    ...
    bool NeedTruncate(size_t posting_num) const;
...
private:
    ...
    CoarseIndex<SmallBlock> coarse_index_;
    ...
    std::vector<char> rank_score_profile_base_;
    ...
    // dump centroid
    IndexPackage centroid_package_;
