#ifndef __MERCURY_CORE_GROUP_IVF_MERGER_H__
#define __MERCURY_CORE_GROUP_IVF_MERGER_H__
...
/*! Index Merger
 */
class GroupIvfMerger : public Merger
{
public:
    //! Index Merger Pointer
    typedef std::shared_ptr<GroupIvfMerger> Pointer;
    ...
    const void *DumpCentroid(size_t *size) override;
...
private:
    ...
    int GenerateSlotsProfile(const GroupIvfIndex* ivf_index);

private:
    GroupIvfIndex merged_index_;
    std::vector<std::vector<std::vector<SlotIndex>>> indexes_slots_profile_; //index->docid->slot indexes
};

MERCURY_NAMESPACE_END(core);

#endif //__MERCURY_CORE_GROUP_IVF_MERGER_H__
