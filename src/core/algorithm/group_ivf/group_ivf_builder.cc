...
//! Initialize Builder
int GroupIvfBuilder::Init(IndexParams &params)
{
    if (!index_) {
...
//! Build the index
int GroupIvfBuilder::AddDoc(docid_t doc_id, uint64_t pk, const std::string &build_str, const std::string &primary_key)
{
    return index_->Add(doc_id, pk, build_str, primary_key);
}

int GroupIvfBuilder::GetRankScore(const std::string &build_str, float *score)
{
...
float GroupIvfBuilder::CalcScore(uint32_t level, SlotIndex label)
{
    return level == sort_build_group_level_ ? log(label + 1) : 0;
}

//! Dump index into file or memory
int GroupIvfBuilder::DumpIndex(const std::string &path, const IndexStorage::Pointer &stg)
{
    // TODO
    return 0;
}

const void *GroupIvfBuilder::DumpIndex(size_t *size)
{
