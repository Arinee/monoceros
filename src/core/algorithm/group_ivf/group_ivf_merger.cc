#include "group_ivf_merger.h"

MERCURY_NAMESPACE_BEGIN(core);

//! Initialize Merger
int GroupIvfMerger::Init(const IndexParams &params) {
    merged_index_.SetIndexParams(params);
    return 0;
}

template <typename T>
int GroupIvfMerger::DoMergeEachIvf(docid_t new_id, T old_id_pk, size_t index_offset,
                              const std::vector<Index::Pointer> &indexes, bool by_pk,
                              GroupIvfIndex* merged_index) {
    ...
    auto ivf_index = dynamic_cast<GroupIvfIndex*>(indexes.at(index_offset).get());
    if (by_pk) {
        if (!ivf_index->WithPk()) {
    ...
    if (merged_index->MultiAgeMode()) {
        merged_index->GetDocCreateTimeProfile().insert(new_id, ivf_index->GetDocCreateTimeProfile().getInfo(old_id));
    }
...
template <typename T>
int GroupIvfMerger::DoMerge(const std::vector<Index::Pointer> &indexes,
    ...
    for (const auto& index : indexes) {
        const GroupIvfIndex* ivf_index = dynamic_cast<GroupIvfIndex*>(index.get());
    ...
    auto front_index = dynamic_cast<GroupIvfIndex*>(indexes.front().get());
...
int GroupIvfMerger::GenerateSlotsProfile(const GroupIvfIndex* ivf_index) {
    ...
    // 用空的slotindex初始化
    slots_profile.resize(ivf_index->GetDocNum(), std::vector<SlotIndex>());
...
int GroupIvfMerger::MergeByDocid(const std::vector<Index::Pointer> &indexes, const MergeIdMap& id_map) {
    return DoMerge(indexes, id_map, false);
}

int GroupIvfMerger::MergeByPk(const std::vector<Index::Pointer> &indexes, const MergePkMap& pk_map) {
    return DoMerge(indexes, pk_map, true);
}

//! Dump index into file or memory
int GroupIvfMerger::DumpIndex(const std::string &path, const IndexStorage::Pointer &stg) {
    return -1;
}

const void * GroupIvfMerger::DumpIndex(size_t *size) {
...
const void *GroupIvfMerger::DumpCentroid(size_t *size)
{
    const void *data = nullptr;
    if (merged_index_.DumpCentroid(data, *size) != 0) {
...
bool GroupIvfMerger::CheckSize(const std::vector<Index::Pointer> &indexes, size_t new_size) const {
    // check
    size_t total_size = 0;
