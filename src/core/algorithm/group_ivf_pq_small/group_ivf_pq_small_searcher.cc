#include "group_ivf_pq_small_searcher.h"
...
void* GroupIvfPqSmallSearcher::BthreadRun(void* message) {
...
void GroupIvfPqSmallSearcher::BatchScore(size_t node_start, size_t node_end,
...
void GroupIvfPqSmallSearcher::BatchScorePq(size_t node_start, size_t node_end,
                                      std::vector<DistNode>* dist_nodes,
...
size_t GroupIvfPqSmallSearcher::GetPostingsNodeCount(const std::vector<CoarseIndex<SmallBlock>::PostingIterator>& ivf_postings,
...
GroupIvfPqSmallSearcher::GroupIvfPqSmallSearcher() {
    if (!index_) {
...
int GroupIvfPqSmallSearcher::Init(IndexParams &params) {
...
int GroupIvfPqSmallSearcher::LoadIndex(const std::string& path) {
    //TODO
    return -1;
}

int GroupIvfPqSmallSearcher::LoadIndex(const void* data, size_t size) {
...
void GroupIvfPqSmallSearcher::SetIndex(Index::Pointer index)
{
    index_ = std::dynamic_pointer_cast<GroupIvfPqSmallIndex>(index);
    dist_scorer_factory_.Init(index_->GetIndexMeta());
}

void GroupIvfPqSmallSearcher::SetBaseDocId(exdocid_t baseDocId)
{
    base_docid_ = baseDocId;
    index_->SetBaseDocid(baseDocId);
}

IndexMeta::FeatureTypes GroupIvfPqSmallSearcher::getFType()
{
    return this->index_->GetIndexMeta().type();
}

int GroupIvfPqSmallSearcher::Search(const QueryInfo& query_info, GeneralSearchContext* context) {
    ...
    std::vector<DistNode> dist_nodes;
    MONITOR_TRANSACTION_LOG(CalcNodeDist(ivf_postings, ivf_postings_group_ids, query_info, dist_nodes), 
...
int GroupIvfPqSmallSearcher::CollectMultiAgeResult(const QueryInfo& query_info,
                                              std::vector<DistNode>& dist_nodes,
...
int GroupIvfPqSmallSearcher::CollectNthTopk(uint32_t start, uint32_t end, uint32_t topk,
                                       std::vector<DistNode>& dist_nodes,
...
int GroupIvfPqSmallSearcher::CalcNodeDist(std::vector<CoarseIndex<SmallBlock>::PostingIterator>& ivf_postings,
                                   std::vector<uint32_t>& ivf_postings_group_ids,
                                   const QueryInfo& query_info,
                                   std::vector<DistNode>& dist_nodes) {
...
int GroupIvfPqSmallSearcher::CollectIvfPostings(std::vector<CoarseIndex<SmallBlock>::PostingIterator>& ivf_postings,
                                          std::vector<uint32_t>& ivf_postings_group_ids,
...
int GroupIvfPqSmallSearcher::CollectGroupHeaps(const QueryInfo& query_info,
                                          std::vector<DistNode>& dist_nodes,
...
int GroupIvfPqSmallSearcher::CollectBasicResult(std::vector<MyHeap<DistNode>>& group_heaps, const std::vector<uint32_t>& topks,
                                        GeneralSearchContext* context) const {
...
int GroupIvfPqSmallSearcher::CollectLeftResult(uint32_t total, std::vector<MyHeap<DistNode>>& group_heaps,
                                        GeneralSearchContext* context) const {
...
int GroupIvfPqSmallSearcher::PostProcess(const QueryInfo& query_info, GeneralSearchContext* context, 
...
void GroupIvfPqSmallSearcher::PushSearchResultToContext(GeneralSearchContext* context, const std::vector<DistNode>& dist_vec) const {
    bool with_pk = index_->WithPk();
