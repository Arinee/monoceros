#include "group_ivf_pq_small_merger.h"

MERCURY_NAMESPACE_BEGIN(core);

//! Initialize Merger
int GroupIvfPqSmallMerger::Init(const IndexParams &params) {
    merged_index_.SetIndexParams(params);
    return 0;
}

template <typename T>
int GroupIvfPqSmallMerger::DoMergeEachIvf(docid_t new_id, T old_id_pk, size_t index_offset,
    ...
    auto ivf_index = dynamic_cast<GroupIvfPqSmallIndex*>(indexes.at(index_offset).get());
    if (by_pk) {
        if (!ivf_index->WithPk()) {
...
template <typename T>
int GroupIvfPqSmallMerger::DoMerge(const std::vector<Index::Pointer> &indexes,
    ...
    for (const auto& index : indexes) {
        const GroupIvfPqSmallIndex* ivf_index = dynamic_cast<GroupIvfPqSmallIndex*>(index.get());
    ...
    auto front_index = dynamic_cast<GroupIvfPqSmallIndex*>(indexes.front().get());
...
int GroupIvfPqSmallMerger::GenerateSlotsProfile(const GroupIvfPqSmallIndex* ivf_index) {
...
int GroupIvfPqSmallMerger::MergeByDocid(const std::vector<Index::Pointer> &indexes, const MergeIdMap& id_map) {
    return DoMerge(indexes, id_map, false);
}

int GroupIvfPqSmallMerger::MergeByPk(const std::vector<Index::Pointer> &indexes, const MergePkMap& pk_map) {
    return DoMerge(indexes, pk_map, true);
}

//! Dump index into file or memory
int GroupIvfPqSmallMerger::DumpIndex(const std::string &path, const IndexStorage::Pointer &stg) {
    return -1;
}

const void * GroupIvfPqSmallMerger::DumpIndex(size_t *size) {
...
bool GroupIvfPqSmallMerger::CheckSize(const std::vector<Index::Pointer> &indexes, size_t new_size) const {
    // check
    size_t total_size = 0;
