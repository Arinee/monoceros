...
MatrixPointer GroupIvfPqSmallIndex::NullMatrix() const
{
    return MatrixPointer(nullptr);
}

GroupIvfPqSmallIndex::GroupIvfPqSmallIndex()
{
    SetThreadEnv();
}

int GroupIvfPqSmallIndex::Load(const void *data, size_t size)
{
    LOG_INFO("Begin to Load group_ivf_pq_small_index. size: %lu.", size);
...
int GroupIvfPqSmallIndex::CopyInit(const Index *index, size_t doc_num)
{
    if (!index) {
    ...
    const GroupIvfPqSmallIndex *ivf_index = dynamic_cast<const GroupIvfPqSmallIndex *>(index);
...
int GroupIvfPqSmallIndex::Create(IndexParams &index_params)
{
...
MatrixPointer GroupIvfPqSmallIndex::DoLoadCentroidMatrix(const std::string &file_path, size_t dimension, size_t element_size,
...
bool GroupIvfPqSmallIndex::StrToValue(const std::string &source, void *value) const
{
...
bool GroupIvfPqSmallIndex::InitIvfCentroidMatrix(const std::string &centroid_dir)
{
...
bool GroupIvfPqSmallIndex::InitPqCentroidMatrix(const IndexParams &param)
{
...
bool GroupIvfPqSmallIndex::ResolveGroupFile(const std::string &meta_path, std::vector<GroupInfo> &groups,
                                       std::unordered_map<GroupInfo, uint32_t, GroupHash, GroupCmp> &group_centroids)
{
    // TODO, resolve group
    std::string file_content;
...
/// add a new vector
int GroupIvfPqSmallIndex::Add(docid_t doc_id, pk_t pk, const std::string &query_str, const std::string &primary_key)
{
...
SlotIndex GroupIvfPqSmallIndex::GetNearestGroupLabel(const void *data, size_t /* size */, gindex_t group_index,
...
bool GroupIvfPqSmallIndex::IsFull() const
{
...
float GroupIvfPqSmallIndex::GetRankScore(docid_t doc_id)
{
    return *(SlotIndex *)rank_score_profile_.getInfo(doc_id);
}

// 计算给定内存下能够存储多少文档
size_t GroupIvfPqSmallIndex::MemQuota2DocCount(size_t memQuota, size_t /* elemSize */) const
{
...
int GroupIvfPqSmallIndex::Dump(const void *&data, size_t &size)
{
...
int64_t GroupIvfPqSmallIndex::UsedMemoryInCurrent() const
{
...
bool GroupIvfPqSmallIndex::NeedParralel(size_t centroid_num) const
{
...
int GroupIvfPqSmallIndex::PushQueue(const std::vector<CentroidInfo> &infos, CentroidQueue &cq) const
{
...
void *GroupIvfPqSmallIndex::BthreadRun(void *message)
{
...
int GroupIvfPqSmallIndex::CalcCentroidScore(CentroidResource &centroid_resource, size_t start, size_t end,
...
int GroupIvfPqSmallIndex::CalcFineCentroidScore(CentroidResource &centroid_resource, size_t start, size_t end,
...
int GroupIvfPqSmallIndex::SearchIvf(gindex_t group_index, const void *datas, size_t size, size_t query_dimension,
...
void GroupIvfPqSmallIndex::RecoverPostingFromSlot(std::vector<CoarseIndex<SmallBlock>::PostingIterator> &postings,
...
int GroupIvfPqSmallIndex::SearchGroup(docid_t docid, std::vector<GroupInfo> &group_infos, std::vector<int> &labels) const
{
