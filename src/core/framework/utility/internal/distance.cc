...
static inline float squared_euclidean_distance_v128(const half_float::half *lhs,
                                                    const half_float::half *rhs,
                                                    size_t size)
{
    // lhs: query; rhs: goods
    const half_float::half *last = lhs + size;
    const half_float::half *last_aligned = lhs + ((size >> 3) << 3);
    float result = 0.0f;
    ...
    if (((uintptr_t)lhs & 0xf) == 0 && ((uintptr_t)rhs & 0xf) == 0) {
        for (; lhs != last_aligned; lhs += 8, rhs += 8) {
            ...
            __m128 xmm0 =
                _mm_sub_ps(_mm_cvtph_ps(_mm_unpacklo_epi64(ixmm0, ixmm_zero)),
                           _mm_cvtph_ps(_mm_unpacklo_epi64(ixmm1, ixmm_zero)));
            __m128 xmm1 =
                _mm_sub_ps(_mm_cvtph_ps(_mm_unpackhi_epi64(ixmm0, ixmm_zero)),
                           _mm_cvtph_ps(_mm_unpackhi_epi64(ixmm1, ixmm_zero)));
        ...
        for (; lhs != last_aligned; lhs += 8, rhs += 8) {
            __m128i ixmm0 = _mm_loadu_si128((const __m128i *)lhs);
            __m128i ixmm1 = _mm_loadu_si128((const __m128i *)rhs);
            __m128 xmm0 =
                _mm_sub_ps(_mm_cvtph_ps(_mm_unpacklo_epi64(ixmm0, ixmm_zero)),
                           _mm_cvtph_ps(_mm_unpacklo_epi64(ixmm1, ixmm_zero)));
            __m128 xmm1 =
...
static inline float inner_product_v128(const half_float::half *lhs, const half_float::half *rhs,
                                       size_t size)
{
    const half_float::half *last = lhs + size;
    const half_float::half *last_aligned = lhs + ((size >> 3) << 3);
    ...
    if (((uintptr_t)lhs & 0xf) == 0 && ((uintptr_t)rhs & 0xf) == 0) {
        for (; lhs != last_aligned; lhs += 8, rhs += 8) {
            __m128i ixmm0 = _mm_load_si128((const __m128i *)lhs);
            __m128i ixmm1 = _mm_load_si128((const __m128i *)rhs);
            xmm_sum1 = _mm_fmadd_ps(
                _mm_cvtph_ps(_mm_unpacklo_epi64(ixmm0, ixmm_zero)),
                _mm_cvtph_ps(_mm_unpacklo_epi64(ixmm1, ixmm_zero)), xmm_sum1);
            xmm_sum2 = _mm_fmadd_ps(
                _mm_cvtph_ps(_mm_unpackhi_epi64(ixmm0, ixmm_zero)),
                _mm_cvtph_ps(_mm_unpackhi_epi64(ixmm1, ixmm_zero)), xmm_sum2);
        }
    } else {
        for (; lhs != last_aligned; lhs += 8, rhs += 8) {
            __m128i ixmm0 = _mm_loadu_si128((const __m128i *)lhs);
...
static inline float squared_euclidean_distance_v256(const half_float::half *lhs,
                                                    const half_float::half *rhs,
                                                    size_t size)
{
    const half_float::half *last = lhs + size;
    const half_float::half *last_aligned = lhs + ((size >> 4) << 4);
    float result = 0.0f;

    __m256 ymm_sum1 = _mm256_setzero_ps();
    __m256 ymm_sum2 = _mm256_setzero_ps();

    // load_si128 只需要16字节对齐就好了 考虑换成load_si256? 会不会更快?
    if (((uintptr_t)lhs & 0xf) == 0 && ((uintptr_t)rhs & 0xf) == 0) {
        for (; lhs != last_aligned; lhs += 16, rhs += 16) {
            __m256 ymm0 = _mm256_sub_ps(
                _mm256_cvtph_ps(
            ...
            ymm_sum1 = _mm256_fmadd_ps(ymm0, ymm0, ymm_sum1);
            __m256 ymm1 =
                _mm256_sub_ps(_mm256_cvtph_ps(_mm_load_si128(
                                  reinterpret_cast<const __m128i *>(lhs + 8))),
                              _mm256_cvtph_ps(_mm_load_si128(
                                  reinterpret_cast<const __m128i *>(rhs + 8))));
            ymm_sum2 = _mm256_fmadd_ps(ymm1, ymm1, ymm_sum2);
        }
        ...
        for (; lhs != last_aligned; lhs += 16, rhs += 16) {
            ...
            ymm_sum1 = _mm256_fmadd_ps(ymm0, ymm0, ymm_sum1);
            __m256 ymm1 =
                _mm256_sub_ps(_mm256_cvtph_ps(_mm_loadu_si128(
                                  reinterpret_cast<const __m128i *>(lhs + 8))),
...
static inline float inner_product_v256(const half_float::half *lhs, const half_float::half *rhs,
                                       size_t size)
{
    const half_float::half *last = lhs + size;
    const half_float::half *last_aligned = lhs + ((size >> 4) << 4);
    ...
    if (((uintptr_t)lhs & 0xf) == 0 && ((uintptr_t)rhs & 0xf) == 0) {
        for (; lhs != last_aligned; lhs += 16, rhs += 16) {
            ymm_sum1 = _mm256_fmadd_ps(
                _mm256_cvtph_ps(
                    _mm_load_si128(reinterpret_cast<const __m128i *>(lhs))),
                _mm256_cvtph_ps(
                    _mm_load_si128(reinterpret_cast<const __m128i *>(rhs))),
                ymm_sum1);
            ymm_sum2 = _mm256_fmadd_ps(
                _mm256_cvtph_ps(
...
float SquaredEuclideanDistance(const half_float::half *lhs, const half_float::half *rhs, size_t size)
{
// #if defined(__AVX512F__)
//     if (size > 15 && CpuFeatures::AVX512F()) {
//         return squared_euclidean_distance_v512(lhs, rhs, size);
...
float InnerProduct(const half_float::half *lhs, const half_float::half *rhs, size_t size)
{
#if defined(__AVX__) && defined(__F16C__)
    if (size > 7 && CpuFeatures::AVX()) {
        return inner_product_v256(lhs, rhs, size);
