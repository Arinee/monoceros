 ...
 *   \date     Dec 2017
 *   \version  1.0.0
 *   \brief    Implementation of CRC32C (Castagnoli)
 */
 ...
 */
static inline uint32_t crc32c_slicing8(uint32_t crc, const void *data,
                                       size_t len)
{
...
#ifdef __SSE4_2__
#ifdef PLATFORM_M64
static inline uint32_t crc32c_sse42(uint32_t crc, const void *data, size_t len)
{
...
{
#ifdef __SSE4_2__
    if (CpuFeatures::SSE4_2()) {
        return crc32c_sse42(crc, data, len);
    }
#endif
    return crc32c_slicing8(crc, data, len);
}
