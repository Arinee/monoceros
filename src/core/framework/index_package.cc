...
bool IndexPackage::load(const void* data, size_t size, bool checksum_) {
    if (size == 0) {
        LOG_ERROR("size is zero.");
        return false;
    }

    const IndexFormat::Meta *meta = IndexFormat::Cast(data, size);
    if ((!meta) || (checksum_ && !meta->check())) {
        LOG_ERROR("Invalid meta.");
        return false;
    }

    LOG_INFO("Seg count: %lu", meta->getSegmentCount());
    for (size_t i = 0, seg_count = meta->getSegmentCount(); i < seg_count; ++i) {
        const IndexFormat::SegmentMeta *segment = meta->getSegment(i);
        size_t data_size = static_cast<size_t>(segment->getDataSize());

        if (data_size &&
            segment->getDataIndex() + data_size <= meta->getPayloadSize()) {
            const uint8_t * data_p = segment->getData(meta->getPayload());
            if (std::strcmp(segment->getSegmentId(), "fastscan_index.dat") == 0) {  
                _aligned32seg = (void *)data_p;
                _aligned32size = data_size;
            } else {
                _vec.emplace_back(segment->getSegmentId(),
                              data_p,
                              (size_t)data_size);
...
bool IndexPackage::dump(const void*& data, size_t& size, bool checksum) {
    LOG_INFO("Dumping...");
    size_t total_size = 0;
    for (auto iter = _vec.begin(); iter != _vec.end(); ++iter) {
        total_size +=
    ...
    if (_dumpPtr) std::free(_dumpPtr);
    _dumpPtr = static_cast<char*>(aligned_alloc(32, total_size));
    if (!_dumpPtr) {
        std::cerr << "Failed to alloc memory: " << total_size << std::endl;
        return false;
    }

    //write back to callser
    size = total_size;
    data = _dumpPtr;
    auto currPtr = _dumpPtr;
    ...
    // Write head into target ptr
    std::memcpy(currPtr, buffer.data(), buffer.size());
    currPtr += buffer.size();
    ...
    for (size_t i = 0; i < _vec.size(); ++i) {
        ...
        std::memcpy(currPtr, src.getData(), src.getDataSize());
        currPtr += src.getDataSize();

        // Write the padding if need
        if (padding_size) {
            padding.resize(padding_size);
            std::memcpy(currPtr, padding.data(), padding.size());
            currPtr += padding.size();
        }
    }
