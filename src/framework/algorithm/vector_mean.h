...
#ifndef __MERCURY_ALGORITHM_VECTOR_MEAN_H__
#define __MERCURY_ALGORITHM_VECTOR_MEAN_H__
...
public:
    ...
    virtual void mean(std::string *out) const
    {
        ...
        vec.resize(_accums.size());
        for (size_t i = 0; i < _accums.size(); ++i) {
            vec[i] = static_cast<T>(_count / _accums[i]);
        }
    }
...
public:
    ...
    virtual void mean(std::string *out) const
    {
        ...
        vec.resize(_accums.size());
        for (size_t i = 0; i < _accums.size(); ++i) {
            vec[i] = static_cast<T>(std::pow(_accums[i], 1.0 / _count));
        }
    }
...
public:
    ...
    virtual bool merge(const VectorMean &rhs)
    {
        const BinaryVectorMean &src =
            dynamic_cast<const BinaryVectorMean &>(rhs);
...
#endif // __MERCURY_ALGORITHM_VECTOR_MEAN_H__
